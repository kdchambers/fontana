...
fn FreetypeHarfbuzzImplementation(comptime types: Types) type {
    return struct {
        const Self = @This();
        const Pen = PenGenerator(@This(), types);
        ...
        inline fn writeFreetypeHarfbuzz(
            ...
            while (i < buffer_length) : (i += 1) {
                ...
                if (codepoint != ' ') {
                    ...
                    std.log.info("codepoint {c} xy {d} {d}", .{ codepoint, screen_extent.x, screen_extent.y });
...
fn FreetypeImplementation(comptime types: Types) type {
    return struct {
        const Self = @This();
        const Pen = PenGenerator(@This(), types);
        ...
        inline fn createPenFreetype(
            ...
            pen.codepoints = codepoints;
            for (codepoints) |codepoint, codepoint_i| {
                const err_code = self.impl.loadCharFn(face, @intCast(u32, codepoint), .{ .render = true });
                std.debug.assert(err_code == 0);
                const bitmap = face.glyph.bitmap;
                const bitmap_height = bitmap.rows;
                const bitmap_width = bitmap.width;
        ...
        inline fn writeFreetype(
            ...
            for (codepoints) |codepoint| {
                const err_code = self.internal.loadCharFn(face, @intCast(u32, codepoint), .{ .render = true });
                std.debug.assert(err_code == 0);
                const bbox_ymax = @intToFloat(f32, face.bbox.y_max) / 64;
                const glyph_width = @intToFloat(f32, face.glyph.metrics.width) / 64;
                const advance = @intToFloat(f32, face.glyph.metrics.horiAdvance) / 64;
                const x_offset: f32 = (advance - glyph_width) / 2.0;
                const y_offset: f32 = (bbox_ymax - face.glyph.metrics.horiBearingY) / 64;
                const glyph_texture_extent = self.textureExtentFromCodepoint(codepoint);
                const texture_extent = types.Extent2DNative{
                    .x = @intToFloat(f32, glyph_texture_extent.x) / texture_width_height,
                    .y = @intToFloat(f32, glyph_texture_extent.y) / texture_width_height,
                ...
                try writer_interface.write(screen_extent, texture_extent);
                cursor.x += advance * screen_scale.horizontal;
            }
        }
    };
}

fn FontanaImplementation(comptime types: Types) type {
    return struct {
        const Pen = PenGenerator(@This(), types);
        ...
        inline fn createPen(
            ...
            const font: *otf.FontInfo = &self.font;
            self.font_scale = otf.fUnitToPixelScale(size_point, points_per_pixel, font.units_per_em);
            var pen: Pen = undefined;
            pen.font = self;
            pen.atlas = atlas_ref;
        ...
        inline fn write(
            ...
            while (i < codepoints.len) : (i += 1) {
                ...
                const glyph_texture_extent = self.textureExtentFromCodepoint(codepoint);
                const texture_extent = types.Extent2DNative{
                    .x = @floatCast(f32, @intToFloat(f64, glyph_texture_extent.x) / texture_width_height),
                    .y = @floatCast(f32, @intToFloat(f64, glyph_texture_extent.y) / texture_width_height),
                    .width = @floatCast(f32, @intToFloat(f64, glyph_texture_extent.width) / texture_width_height),
                    .height = @floatCast(f32, @intToFloat(f64, glyph_texture_extent.height) / texture_width_height),
                };

                const x: f64 = cursor.x;
                const y: f64 = cursor.y + (glyph_metrics.descent * screen_scale.vertical);
                const width: f64 = @intToFloat(f64, glyph_texture_extent.width) * screen_scale.horizontal;
                const height: f64 = @intToFloat(f64, glyph_texture_extent.height) * screen_scale.vertical;

                const screen_extent = types.Extent2DNative{
                    .x = @floatCast(f32, x),
                    .y = @floatCast(f32, y),
                    .width = @floatCast(f32, width),
                    .height = @floatCast(f32, height),
                };
        ...
        pub fn writeCentered(
            ...
            while (i < codepoints.len) : (i += 1) {
                ...
                const glyph_metrics = self.font.glyphMetricsFromCodepoint(codepoint);
                std.log.info("Codepoint: {c} Descent: {d}", .{ codepoint, glyph_metrics.descent });
...
inline fn snap(value: f32) f32 {
    const step = 1.0 / 256.0;
    const half_step = 1.0 / 512.0;
    const overshoot: f32 = @rem(value, step);
    std.debug.assert(overshoot >= 0);
    std.debug.assert(overshoot <= step);
    if (overshoot == 0) return value;

    return if (overshoot >= half_step)
        value + (step - overshoot)
    else
        value - (step - overshoot);
}
...
pub fn Font(comptime backend: Backend, comptime types: Types) type {
    return struct {
        const BackendImplementation = switch (backend) {
            .freetype_harfbuzz => FreetypeHarfbuzzImplementation(types),
            .freetype => FreetypeImplementation(types),
            .fontana => FontanaImplementation(types),
        };

        pub const Pen = PenGenerator(BackendImplementation, types);
        ...
        pub fn createPen(
            ...
            return self.internal.createPen(
                PixelType,
                allocator,
                size.point,
                points_per_pixel,
...
pub fn PenGenerator(
    comptime BackendImplementation: type,
    comptime types: Types,
) type {
    return struct {
